...
void HashTableDirectoryPage::IncrGlobalDepth() {
  if (global_depth_ == MAX_BUCKET_DEPTH) {
    ...
    for (uint32_t i = pre_size; i < 2 * pre_size; i++) {
...
auto HashTableDirectoryPage::GetLocalDepth(uint32_t bucket_idx) -> uint32_t { return this->local_depths_[bucket_idx]; }
...
void HashTableDirectoryPage::IncrLocalDepth(uint32_t bucket_idx) {
  if (this->local_depths_[bucket_idx] + 1 > static_cast<int8_t>(this->global_depth_)) {
    return;
  } else {
